...
const T = {
	ar: {
		...
		introText:
			'تظهر صورة وكلمة ناقصة، يجب عليك النظر إلى الصورة ثم نطق الكلمة المناسبة.',
		start: 'إبدأ',
		level: 'المستوى',
		score: 'النتيجة',
		item: 'العنصر',
	...
	fr: {
		...
		introText:
			"Une image et un mot incomplet s’affichent, vous devez regarder l'image puis prononcer le mot qui convient.",
		start: 'Commencer',
		level: 'Niveau',
		score: 'Score',
		item: 'Élément',
		...
		practiceHint:
			'Appuyez sur le micro puis prononcez clairement le mot complet.',
	...
	en: {
		...
		introText:
			'An image and an incomplete word are shown. You must look at the image, then pronounce the matching word.',
		start: 'Start',
		level: 'Level',
		score: 'Score',
		item: 'Item',
...
function pickLevelItems(levelIndex) {
	...
	let available = WORD_KEYS.filter(
...
function startlangCompTimer(totalSec) {
  clearInterval(_langTimer);
  _langTimeLeft = totalSec;
  _langTimer = setInterval(() => {
    _langTimeLeft -= 1;
    const el = document.getElementById("lang-comp-timer-val");
    if (el) el.textContent = _langTimeLeft;
    const pill = document.getElementById("lang-comp-timer-pill");
