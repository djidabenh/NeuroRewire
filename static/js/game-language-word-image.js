...
const IMAGE_LIBRARY = {
	...
	phone: {
		...
		pronAr: 'تيليفون',
		pronFr: 'tilifoun',
		accepted: ['tilifoun', 'telefon', 'tilيفون', 'تيليفون', 'téléphone'],
	},
	cup: {
...
function pickLevelItems(levelIndex) {
	...
	let available = IMAGE_KEYS.filter(
...
function startlangWITimer(totalSec) {
  clearInterval(_langTimer);
  _langTimeLeft = totalSec;
  _langTimer = setInterval(() => {
    _langTimeLeft -= 1;
    const el = document.getElementById("lang-wi-timer-val");
    if (el) el.textContent = _langTimeLeft;
    const pill = document.getElementById("lang-wi-timer-pill");
