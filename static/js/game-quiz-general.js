...
// Niveau 1 : 3 questions faciles, sans chrono    → très facile
// Niveau 2 : 3 questions faciles, sans chrono
// Niveau 3 : 4 questions faciles+moyennes, 30s
// Niveau 4 : 4 questions moyennes, 25s          → modéré
// Niveau 5 : 5 questions moyennes+difficiles, 20s → difficile
// Niveau 6 : 5 questions difficiles, 15s          → très difficile
// Progression de difficulté :
// Niveau 1 : 3 questions, pas de timer             → très facile
// Niveau 2 : 4 questions, 40s
// Niveau 3 : 4 questions, 30s
// Niveau 4 : 5 questions, 22s
// Niveau 5 : 5 questions, 16s                      → difficile
// Niveau 6 : 6 questions, 12s                      → très difficile
const LEVELS = [3, 4, 4, 5, 5, 6];
const LEVEL_TIME_LIMITS = [20, 20, 20, 20, 20, 20]; // 0 = pas de limite
...
function renderQuestion() {
  updateTexts();
  if (_quizGeneralTimer) { clearInterval(_quizGeneralTimer); _quizGeneralTimer = null; }

  const root = document.getElementById("quiz-general-root");
  const q = levelQuestions[questionCursor];
  const totalLevel = levelQuestions.length;
  const timeLimitSec = LEVEL_TIME_LIMITS[currentLevelIndex] || 0;
  ...
  root.innerHTML = `
    <div class="quiz-shell-card">
      <div class="quiz-meta-row">
        ...
        ${timeLimitSec > 0 ? `<div class="quiz-mini-badge" id="quiz-gen-timer" style="font-weight:700;color:var(--primary)">⏱ <span id="quiz-gen-timer-val">${timeLimitSec}</span>s</div>` : ""}
      </div>
  ...
  if (timeLimitSec > 0) {
    ...
    _quizGeneralTimer = setInterval(() => {
      timeLeft -= 1;
      const el = document.getElementById("quiz-gen-timer-val");
      if (el) el.textContent = timeLeft;
      if (timeLeft <= 0) {
        clearInterval(_quizGeneralTimer); _quizGeneralTimer = null;
        // Auto-skip as wrong answer
        ...
        if (fb) { fb.textContent = (tt("timeUp") || "Temps écoulé !"); fb.className = "game-feedback bad"; }
        setTimeout(() => {
          questionCursor += 1;
