...
// Niveau 1 : 3 cibles / 8 totales, 3 couleurs        → très facile (ratio 38%, peu de distracteurs)
// Niveau 2 : 3 cibles / 10 totales, 4 couleurs       → ratio 30%
// Niveau 3 : 4 cibles / 12 totales, 4 couleurs       → ratio 33%
// Niveau 4 : 4 cibles / 16 totales, 5 couleurs       → ratio 25% + plus grand
// Niveau 5 : 5 cibles / 18 totales, 5 couleurs       → difficile (ratio 28%)
// Niveau 6 : 5 cibles / 22 totales, 5 couleurs       → très difficile (ratio 23%, très dense)
// Progression de difficulté :
// Niveau 1 : 8 cibles, 3 à toucher, 3 couleurs, pas de timer    → très facile
// Niveau 2 : 10 cibles, 3 à toucher, 4 couleurs, 20s
// Niveau 3 : 12 cibles, 4 à toucher, 4 couleurs, 18s
// Niveau 4 : 16 cibles, 4 à toucher, 5 couleurs, 15s
// Niveau 5 : 18 cibles, 5 à toucher, 5 couleurs, 12s            → difficile
// Niveau 6 : 22 cibles, 5 à toucher, 5 couleurs, 9s             → très difficile
const LEVELS = [
  { totalTargets: 8,  targetCount: 3, colors: ["red", "blue", "green"],                           timeLimitSec: 0  },
  { totalTargets: 10, targetCount: 3, colors: ["red", "blue", "green", "yellow"],                 timeLimitSec: 20 },
  { totalTargets: 12, targetCount: 4, colors: ["red", "blue", "green", "yellow"],                 timeLimitSec: 18 },
  { totalTargets: 16, targetCount: 4, colors: ["red", "blue", "green", "yellow", "purple"],       timeLimitSec: 15 },
  { totalTargets: 18, targetCount: 5, colors: ["red", "blue", "green", "yellow", "purple"],       timeLimitSec: 12 },
  { totalTargets: 22, targetCount: 5, colors: ["red", "blue", "green", "yellow", "purple"],       timeLimitSec: 9  }
];
...
function startTapTimer(totalSec) {
  clearInterval(_tapTimer);
  _tapTimeLeft = totalSec;
  _tapTimer = setInterval(() => {
    _tapTimeLeft -= 1;
    const el = document.getElementById("tap-timer-val");
    if (el) el.textContent = _tapTimeLeft;
    const pill = document.getElementById("tap-timer-pill");
    if (pill && _tapTimeLeft <= 5) pill.style.color = "var(--danger, #e74c3c)";
    if (_tapTimeLeft <= 0) {
      clearInterval(_tapTimer);
      if (!currentLevelData.locked) {
...
function renderBoard() {
  ...
  // Start level timer
  const lvlCfg = LEVELS[currentLevelIndex];
  if (lvlCfg && lvlCfg.timeLimitSec > 0 && !currentLevelData.locked) {
    if (_tapTimeLeft <= 0) startTapTimer(lvlCfg.timeLimitSec);
  }
