...
const FRUIT_LABELS = {
  ...
  strawberry: { ar: "فراولة", fr: "fraises", en: "strawberries" },
  watermelon: { ar: "بطيخ", fr: "pastèques", en: "watermelons" },
  pineapple: { ar: "أناناس", fr: "ananas", en: "pineapples" },
  cherry: { ar: "كرز", fr: "cerises", en: "cherries" }
};


// Progression de difficulté :
// Niveau 1 : 6 images, 2 cibles identiques,  → très facile (ratio 2/6 = 33%)
// Niveau 2 : 9 images, 2 cibles              → ratio 22%
// Niveau 3 : 12 images, 3 cibles             → ratio 25%
// Niveau 4 : 16 images, 3 cibles             → ratio 19%
// Niveau 5 : 18 images, 4 cibles             → difficile (ratio 22% + plus grand)
// Niveau 6 : 20 images, 4 cibles             → très difficile (ratio 20% + énorme grille)
// Progression de difficulté :
// Niveau 1 : 6 images, 2 cibles, pas de timer      → très facile
// Niveau 2 : 9 images, 2 cibles, 30s               → facile
// Niveau 3 : 12 images, 3 cibles, 25s
// Niveau 4 : 16 images, 3 cibles, 20s
// Niveau 5 : 18 images, 4 cibles, 15s              → difficile
// Niveau 6 : 20 images, 4 cibles, 10s              → très difficile
const ROUNDS = [
  { total: 6,  targets: 2, timeLimitSec: 10  },
  { total: 9,  targets: 2, timeLimitSec: 8 },
  { total: 12, targets: 3, timeLimitSec: 8 },
  { total: 16, targets: 3, timeLimitSec: 6 },
  { total: 18, targets: 4, timeLimitSec: 6 },
  { total: 20, targets: 4, timeLimitSec: 6 }
];
...
function startClickTargetTimer(totalSec) {
  clearInterval(_clickTargetTimer);
  // Only start if not already running at this value
  if (_clickTargetTimeLeft <= 0 || _clickTargetTimeLeft > totalSec) {
    _clickTargetTimeLeft = totalSec;
  }
  _clickTargetTimer = setInterval(() => {
    _clickTargetTimeLeft -= 1;
    const el = document.getElementById("click-target-timer-val");
    if (el) el.textContent = _clickTargetTimeLeft;
    const pill = document.getElementById("click-target-timer-pill");
    if (pill) {
      if (_clickTargetTimeLeft <= 5) pill.style.color = "var(--danger, #e74c3c)";
    }
    if (_clickTargetTimeLeft <= 0) {
      clearInterval(_clickTargetTimer);
      if (!currentRoundData.locked) {
        // Time's up → auto finish round with no bonus
...
function stopClickTargetTimer() {
  clearInterval(_clickTargetTimer);
  _clickTargetTimeLeft = 0;
}
...
function renderTargetBoard() {
  ...
  root.innerHTML = `
    ...
    <div class="memory-pairs-stats">
      ...
      ${ROUNDS[currentRoundIndex] && ROUNDS[currentRoundIndex].timeLimitSec > 0 ? `<div class="game-pill" id="click-target-timer-pill" style="font-weight:700;color:var(--primary)">⏱ <span id="click-target-timer-val">${_clickTargetTimeLeft}</span>s</div>` : ""}
    </div>
  ...
  const roundCfg = ROUNDS[currentRoundIndex];
  if (roundCfg && roundCfg.timeLimitSec > 0 && !currentRoundData.locked) {
    startClickTargetTimer(roundCfg.timeLimitSec);
  }
}
