...
// Niveau 1 : scène 0, 1 cible   → très facile
// Niveau 2 : scène 1, 1 cible (différente)
// Niveau 3 : scène 2, 2 cibles
// Niveau 4 : scène 0, 2 cibles (objets différents)
// Niveau 5 : scène 1, 3 cibles  → difficile
// Niveau 6 : scène 2, 4 cibles  → très difficile
// Progression de difficulté :
// Niveau 1 : 1 cible, pas de timer              → très facile
// Niveau 2 : 1 cible, 30s
// Niveau 3 : 2 cibles, 25s
// Niveau 4 : 2 cibles, 20s
// Niveau 5 : 3 cibles, 16s                      → difficile
// Niveau 6 : 4 cibles, 12s                      → très difficile
const LEVELS = [
  { sceneIndex: 0, timeLimitSec: 10,  targetIds: ["kettle"] },
  { sceneIndex: 1, timeLimitSec: 10, targetIds: ["remote"] },
  { sceneIndex: 2, timeLimitSec: 10, targetIds: ["sun", "bird"] },
  { sceneIndex: 0, timeLimitSec: 10, targetIds: ["cup", "flower"] },
  { sceneIndex: 1, timeLimitSec: 12, targetIds: ["glasses", "house", "dog"] },
  { sceneIndex: 2, timeLimitSec: 12, targetIds: ["watering_can", "flower_pot", "cat_garden", "apple_tree"] }
];
...
function setupLevelData() {
  ...
  const levelTimer = LEVELS[currentLevelIndex]?.timeLimitSec || 0;

if (levelTimer > 0) {
  startSceneTimer(levelTimer);
} else {
  stopSceneTimer();
}
}
...
function startSceneTimer(totalSec) {
  clearInterval(_sceneTimer);
  _sceneTimeLeft = totalSec;
  _sceneTimer = setInterval(() => {
    _sceneTimeLeft -= 1;
    const el = document.getElementById("scene-timer-val");
    if (el) el.textContent = _sceneTimeLeft;
    const pill = document.getElementById("scene-timer-pill");
    if (pill && _sceneTimeLeft <= 5) pill.style.color = "var(--danger, #e74c3c)";
    if (_sceneTimeLeft <= 0) {
      clearInterval(_sceneTimer);
      if (!gameEnded) {
        currentLevelIndex += 1;
...
<div class="sbar scene-sbar">
  ...
  <div style="display:flex;align-items:center;gap:10px;">
    ...
    ${
      ...
      ? `
        <div class="game-pill game-pill-score" id="scene-timer-pill">
          ⏳ <span id="scene-timer-val">${_sceneTimeLeft}</span>s
        </div>
      `
      : ""
    }
