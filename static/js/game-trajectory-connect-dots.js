...
// Niveau 1 : 3 points (2 segments), grande tolérance 42px  → très facile
// Niveau 2 : 4 points (3 segments), tolérance 36px
// Niveau 3 : 5 points (4 segments), tolérance 30px
// Niveau 4 : 6 points (5 segments), tolérance 26px         → modéré
// Niveau 5 : 7 points (6 segments), tolérance 22px         → difficile
// Niveau 6 : 9 points (8 segments), tolérance 18px         → très difficile
// Progression: plus de points, tolérance réduite, countdown
const LEVELS = [
  { timeLimitSec: 25,  tolerance: 42, points: [[130,240],[310,240],[450,120]] },
  { timeLimitSec: 22, tolerance: 34, points: [[120,240],[220,130],[340,240],[460,120]] },
  { timeLimitSec: 20, tolerance: 28, points: [[100,250],[180,120],[280,210],[370,90],[470,220]] },
  { timeLimitSec: 18, tolerance: 22, points: [[90,250],[170,110],[250,230],[330,100],[410,210],[490,120]] },
  { timeLimitSec: 15, tolerance: 18, points: [[85,255],[155,120],[220,240],[295,90],[370,235],[445,115],[520,220]] },
  { timeLimitSec: 12, tolerance: 14, points: [[70,260],[130,120],[200,245],[260,100],[325,225],[390,110],[455,245],[520,135],[560,220]] }
];
...
function renderBoard(){
  ...
  root.innerHTML = `
    <div class="trajectory-shell-card">
      ...
      ${LEVELS[currentLevelIndex] && LEVELS[currentLevelIndex].timeLimitSec > 0 ? `<div class="game-pill" id="dots-t-pill" style="font-weight:700;color:var(--primary)">⏱ <span id="dots-t-val">${_tLeft || LEVELS[currentLevelIndex].timeLimitSec}</span>s</div>` : ""}
      </div>
...
onAuthStateChanged(auth, (user) => {
  ...
  _t = setInterval(() => {
    _tLeft--;
    const el = document.getElementById("dots-t-val");
    if (el) el.textContent = _tLeft;
    const pill = document.getElementById("dots-t-pill");
    if (pill && _tLeft <= 5) pill.style.color = "var(--danger, #e74c3c)";
    if (_tLeft <= 0) {
      clearInterval(_t);
      _tLeft = 0;
      started = false; currentLevelIndex++;
      if (currentLevelIndex < LEVELS.length) renderCurrentScreen();
      else finishGame();
    }
  }, 1000);
}
