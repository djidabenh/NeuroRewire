...
// Niveau 1 : couloir large 48px, 4 virages  → très facile
// Niveau 2 : couloir 42px, 5 virages
// Niveau 3 : couloir 36px, 6 virages
// Niveau 4 : couloir 30px, 7 virages        → modéré
// Niveau 5 : couloir 24px, 9 virages        → difficile
// Niveau 6 : couloir 20px, 11 virages       → très difficile (précision maximale)
// Progression: couloir large→étroit, plus de segments, countdown
const LEVELS = [
  { width: 48, timeLimitSec: 30,  points: [[65,200],[200,200],[200,100],[380,100],[380,220],[540,220]] },
  { width: 42, timeLimitSec: 25, points: [[60,250],[160,250],[160,100],[280,100],[280,210],[420,210],[420,120],[540,120]] },
  { width: 36, timeLimitSec: 20, points: [[65,180],[150,180],[150,70],[250,70],[250,250],[370,250],[370,120],[500,120],[500,220]] },
  { width: 30, timeLimitSec: 17, points: [[60,260],[140,260],[140,80],[220,80],[220,210],[310,210],[310,100],[430,100],[430,250],[540,250]] },
  { width: 24, timeLimitSec: 15, points: [[50,260],[115,260],[115,70],[195,70],[195,170],[280,170],[280,90],[375,90],[375,250],[465,250],[465,130],[545,130]] },
  { width: 20, timeLimitSec: 15, points: [[45,250],[110,250],[110,65],[185,65],[185,145],[255,145],[255,85],[340,85],[340,230],[415,230],[415,105],[490,105],[490,200],[555,200]] }
];
...
function _startT(sec) {
  ...
  _t = setInterval(() => {
    _tLeft--;
    const el = document.getElementById("maze-t-val");
    if (el) el.textContent = _tLeft;
    const pill = document.getElementById("maze-t-pill");
    if (pill && _tLeft <= 5) pill.style.color = "var(--danger, #e74c3c)";
    if (_tLeft <= 0) {
      clearInterval(_t);
      _tLeft = 0;
      currentLevelIndex++;
      if (currentLevelIndex < LEVELS.length) { resetMazeState(); renderBoard(); }
      else finishGame();
    }
  }, 1000);
}
...
function renderBoard(){
  ...
  root.innerHTML = `
    <div class="trajectory-shell-card">
      ...
      ${LEVELS[currentLevelIndex] && LEVELS[currentLevelIndex].timeLimitSec > 0 ? `<div class="game-pill" id="maze-t-pill" style="font-weight:700;color:var(--primary)">⏱ <span id="maze-t-val">${_tLeft || LEVELS[currentLevelIndex].timeLimitSec}</span>s</div>` : ""}
      </div>
